...
| `report_share` | Generated reports stored as JSON with a UUID token, expiry, and `viewed_at` read receipt |
| `menstrual_cycle` | Cycle start dates and lengths for female athletes — used for phase display on the dashboard |
...
Reports are saved to the `report_share` table with a UUID token. The shareable URL `/report/[token]` requires no login. The first page view records a `viewed_at` timestamp (read receipt). Expiry can be extended by 30 days via `PATCH /api/report-share`.
